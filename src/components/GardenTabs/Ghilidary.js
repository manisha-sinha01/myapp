...
import { Text,View,ListView,ScrollView,TouchableOpacity,StyleSheet,TouchableHighlight,ActivityIndicator} from 'react-native';
import { ListItem,List,Left,Body,Right,Icon,Fab,Button } from 'native-base';
import s from './styles.js';

export default class Ghilidary extends Component {
  constructor(props){
    super(props);
    this.page_length=20,
    this.state = {
      ...
      url: 'https://runga.rungamatteegroup.com/api/resource/Tea Invoice?fields=["name","invpr","invsuf","grade","status","dop","kgs","mark","tea_brand","fineleaf","sold_rate","sale_date","customer"]&filters=[["Tea Invoice", "estatename", "=", "Ghillidary Tea Estate"]]&limit_page_length='+this.page_length,
  ...
  loadMoreData = () => {
    this.page_length = this.page_length + 20;
      ...
      this.setState({ fetching_from_server: true }, () =>
      {
          ...
          {
              fetch('https://runga.rungamatteegroup.com/api/resource/Tea Invoice?fields=["name","invpr","invsuf","grade","status","dop","kgs","mark","tea_brand","fineleaf","sold_rate","sale_date","customer"]&filters=[["Tea Invoice", "estatename", "=", "Ghillidary Tea Estate"]]&limit_page_length='+this.page_length)
  ...
  componentDidMount(){
    ...
    const url= 'https://runga.rungamatteegroup.com/api/resource/Tea Invoice?fields=["name","invpr","invsuf","grade","status","dop","kgs","mark","tea_brand","fineleaf","sold_rate","sale_date","customer"]&filters=[["Tea Invoice", "estatename", "=", "Ghillidary Tea Estate"]]&limit_page_length='+this.page_length;
